...
class SelfEvolution:
    ...
    def __init__(self, orchestrator):
        ...
        # FailureDNA — meta-learning: turns failures into avoidance strategies
        # so the fix loop LEARNS instead of repeating the same mistake.
        try:
            from failure_dna import FailureDNA
            self.failure_dna = FailureDNA()
        except Exception as e:
            logger.warning(f"FailureDNA unavailable: {e}")
            self.failure_dna = None
        BACKUP_DIR.mkdir(parents=True, exist_ok=True)
    def minimal_fix(self, code):
        return code.replace("  ", " ")

    async def _fix_with_dualbrain_or_coder(
        self, fname: str, code: str, issue_desc: str,
        coder, task_name: str, fp
    ):
        """
        DualBrain-powered fix: 3 swarm candidates → scored → best chosen.
        Falls back to single coder.execute() if DualBrain unavailable.
        Returns fixed code string, or None on failure.
        """
        # ── LEARN FROM PAST MISTAKES: ask FailureDNA what to avoid here ──
        avoid_note = ""
        if self.failure_dna:
            try:
                should, strategy = self.failure_dna.should_avoid(f"fix {fname}")
                if should and strategy:
                    avoid_note = f"\n⚠️ LEARNED FROM PAST FAILURE — AVOID THIS:\n{strategy}\n"
                    logger.info(f"  🧫 FailureDNA strategy applied for {fname}")
            except Exception:
                pass

        task = (
            f"Fix this Python file: {fname}\n\n"
            f"Issues: {issue_desc}\n"
            f"{avoid_note}\n"
            f"Current code:\n```python\n{code[:3000]}\n```\n\n"
            f"Rules:\n"
            f"- Keep ALL existing functions and classes (do NOT delete any)\n"
            f"- Fix only the reported issues\n"
            f"- Return ONLY the complete fixed Python code"
        )

        # Try DualBrain first (3 candidates, fitness-scored)
        try:
            from dual_brain_coder import DualBrainCoder
            db = DualBrainCoder()
            loop = __import__("asyncio").get_event_loop()
            result = await loop.run_in_executor(None, lambda: db.code(task, max_turns=2))
            new_code = result.get("final_code", "").strip()
            fitness = result.get("highest_fitness", 0)

            if new_code and fitness >= 0.5:
                logger.info(f"  🧠 DualBrain fix: {fname} | fitness={fitness:.3f}")
                self._record(PHASE_FIX, f"dualbrain_fix {fname}",
                             f"fitness={fitness:.3f}", success=True)
                return new_code
            else:
                logger.warning(f"  ⚠️  DualBrain fitness too low ({fitness:.3f}) → coder fallback")
        except Exception as e:
            logger.debug(f"  DualBrain unavailable: {e} → coder fallback")

        # Fallback: single coder.execute()
        try:
            result = await coder.execute({
                ...
                "data": {
                    "action": "fix",
                    "code": code[:3000],
                    "error": f"Issues: {issue_desc}. Fix all. Keep all functions. Return ONLY code."
                }
            })
    ...
    async def run(self, max_cycles: int = 0, single_task: str = None):
        ...
        while self.running:
            ...
            try:
                ...
                # Phase 8: Skill Hunt — find and install missing capabilities
                if self.cycle % 2 == 0:
                    await self._phase_skill_hunt()

                # Phase 9: Additive Evolution — grow test coverage every cycle
                await self._phase_additive_evolution()
    ...
    async def _phase_fix(self, issues: list, test_results: dict) -> int:
        ...
        # Root-cause analysis on test/import failures via the connected ErrorAnalyzer.
        subs = getattr(self.orc, "subsystems", None)
        if subs and (test_results.get("failures") or test_results.get("import_errors")):
            ea = subs.get("error_analyzer")
            if ea and hasattr(ea, "analyze"):
                for fail in (test_results.get("import_errors") or [])[:3]:
                    try:
                        rc = ea.analyze(str(fail.get("error", fail)))
                        logger.info(f"  🔬 root-cause [{fail.get('module','?')}]: {str(rc)[:80]}")
                    except Exception:
                        pass
        ...
        for item in fixable:
            ...
            # ── SMART BLACKLIST: 2+ consecutive fails → stop wasting cycles ────
            if self._consec_fail.get(fname, 0) >= 2:
                self._blacklist.add(fname)   # skip in ALL future cycles
                logger.warning(f"  ⛔ Blacklisted {fname} — {self._consec_fail[fname]} fails, "
                               f"won't retry (clean code or unfixable). HUMAN REVIEW if needed.")
                self._record(PHASE_FIX, f"blacklist {fname}",
                             f"{self._consec_fail[fname]} consecutive failures", success=False)
                continue
            ...
            if strategy == "safe_mode":
                ...
                new_code = await self._fix_with_dualbrain_or_coder(
                    fname, code, issue_desc, coder, task_name, fp
                )
                if new_code is None:
                    continue
            ...
            if new_code and len(new_code) > 20:
                try:
                    ...
                    if lost:
                        ...
                        self._consec_fail[fname] = self._consec_fail.get(fname, 0) + 1
                        self.adaptive.record_failure(task_name)
                        # ── LEARN: record this failure so next attempt avoids it ──
                        if self.failure_dna:
                            try:
                                self.failure_dna.record(
                                    error_type="deleted_functions",
                                    error_message=f"While fixing {fname}, deleted functions: {', '.join(list(lost)[:5])}",
                                    module=fname,
                                    context=f"fix {fname}",
                                    severity=0.85,   # serious structural error → cross avoidance threshold
                                )
                                logger.info(f"  🧫 FailureDNA learned: don't delete functions in {fname}")
                            except Exception:
                                pass
                        continue
                    fp.write_text(new_code)
        ...
        for ie in import_errors[:4]:
            ...
            if self._consec_fail.get(fname, 0) >= 2:
                ...
                logger.warning(f"  ⛔ Blacklisted {fname} — won't retry")
                continue
    ...
    async def _phase_refactor(self, advisory_issues: list):
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                ...
                # Try DualBrain for refactor (better quality than single shot)
                new_funcs = ""
                try:
                    from dual_brain_coder import DualBrainCoder
                    db = DualBrainCoder()
                    refactor_task = (
                        f"Refactor this Python function to be shorter and cleaner:\n\n"
                        f"```python\n{func_src}\n```\n\n"
                        f"{instructions}\n"
                        f"Return ONLY the refactored Python code."
                    )
                    loop = __import__("asyncio").get_event_loop()
                    db_result = await loop.run_in_executor(
                        None, lambda: db.code(refactor_task, max_turns=1)
                    )
                    new_funcs = db_result.get("final_code", "").strip()
                    if new_funcs:
                        logger.info(f"  🧠 DualBrain refactor: fitness={db_result.get('highest_fitness',0):.2f}")
                except Exception:
                    pass
    ...
    async def _phase_learn(self):
        ...
        # ── Feed the connected capability subsystems (previously orphaned) ──
        subs = getattr(self.orc, "subsystems", None)
        if subs:
            try:
                sll = subs.get("self_learning_loop")
                if sll and hasattr(sll, "learn"):
                    sll.learn({"cycle": self.cycle, "successes": successes,
                               "failures": failures, "quality": round(avg_q, 3)})
            except Exception as e:
                logger.debug(f"self_learning_loop feed: {e}")
            try:
                dream = subs.get("dream_engine")
                if dream and hasattr(dream, "feed"):
                    dream.feed(f"cycle {self.cycle}: {successes} ok, {failures} failed, q={avg_q:.3f}")
            except Exception as e:
                logger.debug(f"dream feed: {e}")
            try:
                sg = subs.get("self_graph")
                if sg and hasattr(sg, "reflect"):
                    sg.reflect()
            except Exception as e:
                logger.debug(f"self_graph reflect: {e}")
    ...
    # ══════════════════════════════════════════════════════════════
    # PHASE 8: SKILL HUNT — Auto-discover and install new capabilities
    # ══════════════════════════════════════════════════════════════

    async def _phase_skill_hunt(self):
        """Phase 8: Autonomously find and install missing skills."""
        logger.info("🎯 Phase 8: Skill Hunt / ਨਵੀਆਂ ਸਮਰੱਥਾਵਾਂ ਲੱਭੋ")
        try:
            from skill_hunter import SkillHunter
            hunter = SkillHunter(self.orc)
            result = await hunter.scan_and_upgrade()
            installed = result.get("installed", [])
            failed = result.get("failed", [])
            logger.info(f"  🎯 Skills installed: {len(installed)} | failed: {len(failed)}")
            if installed:
                logger.info(f"  ✅ New: {', '.join(installed[:5])}")
            self._record("skill_hunt", f"installed:{len(installed)}", str(installed[:3]), success=True)
        except Exception as e:
            logger.warning(f"  ⚠️  Skill hunt error: {e}")

    # ══════════════════════════════════════════════════════════════
    # PHASE 9: ADDITIVE EVOLUTION — grow value, don't just fix
    # Writes NEW passing tests for untested modules → coverage climbs.
    # Purely additive + safe: a generated test is kept ONLY if it passes.
    # ══════════════════════════════════════════════════════════════
    ...
    async def _phase_additive_evolution(self):
        ...
        cov_file = Path("workspace/additive_coverage.json")
        try:
            covered = set(json.loads(cov_file.read_text())) if cov_file.exists() else set()
        except Exception:
            covered = set()

        # Candidate modules: have functions, not already covered/blacklisted, safe
        skip = {"main", "setup_bitnet", "bitnet_run", "__init__"}
        candidates = []
        for f in sorted(glob.glob("*.py")):
            mod = f[:-3]
            if (mod in skip or mod in covered or f.startswith(("test_", "_"))
                    or mod in self._blacklist):
                continue
            try:
                src = open(f).read()
                if "def " in src and len(src) > 100:
                    candidates.append((mod, src))
            except Exception:
                pass

        if not candidates:
            logger.info("  ✨ All modules have generated tests — coverage complete!")
            return

        mod, src = candidates[0]   # one per cycle
        logger.info(f"  ➕ Writing tests for untested module: {mod}")

        prompt = (
            f"Write a pytest test file for this Python module named '{mod}'.\n"
            f"Test ONLY pure/safe functions (no network, files, subprocess, LLM calls).\n"
            f"Import as: from {mod} import <name>, OR `import {mod}`.\n"
            f"Use simple assert-based test functions (def test_xxx).\n"
            f"If nothing is safely testable, return exactly: # no safe tests\n"
            f"Return ONLY Python code.\n\nMODULE:\n```python\n{src[:4000]}\n```"
        )
        try:
            code = await self.orc.get_agent("coder").ask_llm(prompt, max_tokens=2000)
            code = re.sub(r'^```[a-z]*\n|```$', '', code.strip(), flags=re.MULTILINE).strip()
        except Exception as e:
            logger.warning(f"  ⚠️  test-gen failed: {e}")
            return

        if not code or "# no safe tests" in code or "def test" not in code:
            logger.info(f"  — no safe tests for {mod}; marking covered to move on")
            covered.add(mod)
            cov_file.write_text(json.dumps(sorted(covered)))
            return

        # Security scan + syntax check before running
        try:
            ast.parse(code)
        except SyntaxError:
            logger.info(f"  — generated test had bad syntax; skipping {mod}")
            return

        test_path = gen_dir / f"test_{mod}.py"
        test_path.write_text(code)

        # Run ONLY this test file in isolation — keep only if it passes
        try:
            r = subprocess.run(
                [_sys.executable, "-m", "pytest", str(test_path), "-q", "--no-header"],
                capture_output=True, text=True, timeout=90, cwd="."
            )
            passed = r.returncode == 0 and ("passed" in r.stdout or "passed" in r.stderr)
        except Exception as e:
            passed = False
            logger.warning(f"  ⚠️  test run error: {e}")

        if passed:
            covered.add(mod)
            cov_file.write_text(json.dumps(sorted(covered)))
            n = code.count("def test_")
            logger.info(f"  ✅ ADDED {n} passing test(s) for {mod} → coverage {len(covered)} modules")
            self._record("additive", f"tests_added {mod}", f"{n} tests", success=True)
        else:
            test_path.unlink(missing_ok=True)   # discard failing test (stay safe)
            logger.info(f"  ↩️  generated tests for {mod} didn't pass — discarded (no harm)")
            self._record("additive", f"tests_failed {mod}", "discarded", success=False)
