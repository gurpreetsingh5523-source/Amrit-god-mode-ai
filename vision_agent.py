...
class VisionAgent(BaseAgent):
    ...
    async def execute(self, task: dict) -> dict:
        ...
        if action == "describe": return await self._describe(source)
        if action == "ocr":      return await self._ocr(source)
        if action == "frames":   return await self._frames(source, d.get("fps",1))
        if action == "detect":   return await self._detect(source)
        if action == "recall":   return self.ok(observations=self.vmem.search(d.get("query","")))
        return await self._describe(source)

    async def _describe(self, path: str) -> dict:
        if not Path(path).exists(): return self.err(f"Not found: {path}")
        try:
        ...
        except ImportError: return self.err("Pillow required: pip install Pillow")
    ...
    async def _ocr(self, path: str) -> dict:
        ...
        except ImportError: return self.err("pip install pytesseract Pillow")
        except Exception as e: return self.err(str(e))

    async def _frames(self, path: str, fps: int) -> dict:
        try:
            import cv2; out = Path("workspace/frames"); out.mkdir(parents=True,exist_ok=True)
            ...
            while cap.isOpened():
                ...
                if idx % step == 0:
                    ...
                    cv2.imwrite(fp, frame); frames.append(fp)
                idx += 1
            cap.release()
            return self.ok(count=len(frames), frames=frames[:10])
        except ImportError: return self.err("pip install opencv-python")
        except Exception as e: return self.err(str(e))
