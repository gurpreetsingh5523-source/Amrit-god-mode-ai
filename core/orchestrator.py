...
class Orchestrator:
    ...
    async def initialize(self):
        ...
        self._load_agents()
        # Connect previously-orphaned capability engines into the brain.
        try:
            from subsystems import SubsystemRegistry
            self.subsystems = SubsystemRegistry(self)
        except Exception as e:
            self.subsystems = None
            logger.warning(f"subsystems registry failed: {e}")
        self._register_handlers()
        await self.state.set("status", "running")
        self._ready = True
        n_sub = len(self.subsystems.loaded) if self.subsystems else 0
        logger.info(f"GODMODE ready ✅  ({len(self.agents)} agents, {n_sub} subsystems)")
    ...
    def _load_agents(self):
        ...
        self.agents = {
            ...
            "fullstack":  FullStackAgent(self.event_bus, self.state),
            "ui":         UIDesignAgent(self.event_bus, self.state),
            "swift":      SwiftAgent(self.event_bus, self.state),
            "db":         DBAgent(self.event_bus, self.state),
        }
    ...
    async def run_interactive(self):
        ...
        while True:
            ...
            elif cmd in ("screenshot", "ਸਕ੍ਰੀਨਸ਼ਾਟ"):
                await self._run_screenshot()
            elif cmd.startswith("screen ") or cmd.startswith("ਸਕ੍ਰੀਨ "):
                goal = inp.split(" ", 1)[1] if " " in inp else ""
                await self._run_screen_goal(goal)
            elif cmd in ("skills", "ਹੁਨਰ"):
                await self._run_skill_status()
            elif cmd in ("hunt", "ਹੰਟ"):
                await self._run_skill_hunt()
    ...
    async def _run_screenshot(self):
        """Take screenshot and analyze with LLaVA."""
        try:
            from screen_control import screenshot, understand_screen
            print("\n  📸 Taking screenshot...")
            path = screenshot("interactive")
            print(f"  Saved: {path}")
            print("  Analyzing with LLaVA...")
            result = await understand_screen(path)
            desc = result.get("description", result.get("error", "Could not analyze"))
            print(f"\n  🖥  Screen: {desc[:500]}\n")
        except Exception as e:
            logger.error(f"Screenshot failed: {e}")

    async def _run_screen_goal(self, goal: str):
        """Execute a goal autonomously on screen."""
        try:
            from screen_control import do_goal_on_screen
            print(f"\n  🤖 Executing on screen: {goal!r}")
            result = await do_goal_on_screen(goal)
            steps = result.get("steps", 0)
            success = result.get("success", False)
            print(f"  {'✅' if success else '⚠️ '} Completed {steps} steps\n")
        except Exception as e:
            logger.error(f"Screen goal failed: {e}")
    ...
    async def _run_skill_status(self):
        ...
        try:
            ...
            if status['missing_skills']:
                print(f"     Missing:      {', '.join(status['missing_skills'][:5])}")
            print()
        except Exception as e:
            logger.error(f"Skill status failed: {e}")

    async def _run_skill_hunt(self):
        """Autonomously find and install missing skills."""
        try:
            from skill_hunter import SkillHunter
            hunter = SkillHunter(self)
            print("\n  🎯 Hunting for new skills...")
            result = await hunter.scan_and_upgrade()
            installed = result.get("installed", [])
            print(f"  ✅ Installed: {', '.join(installed) if installed else 'none new'}")
            print(f"  Total skills: {result.get('total_skills', 0)}\n")
        except Exception as e:
            logger.error(f"Skill hunt failed: {e}")
