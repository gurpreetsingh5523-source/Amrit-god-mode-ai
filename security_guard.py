...
class SecurityGuard:
    ...
    def scan(self, code: str) -> Tuple[bool, List[str]]:
        ...
        # ── pre-pass: map import aliases so `import os as o; o.system()` can't bypass ──
        alias_to_module = {}        # bound name -> real root module  (import os as o → o:os)
        denied_func_names = set()   # names bound to denied module funcs (from os import system)
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                for alias in node.names:
                    real = alias.name.split(".")[0]
                    bound = (alias.asname or alias.name).split(".")[0]
                    alias_to_module[bound] = real
            elif isinstance(node, ast.ImportFrom):
                mod = (node.module or "").split(".")[0]
                for alias in node.names:
                    if (mod, alias.name) in self.DENY_ATTR:
                        denied_func_names.add(alias.asname or alias.name)
        ...
        for node in ast.walk(tree):
            ...
            # eval(...) / exec(...) / os.system(...) ...
                ...
                # from os import system; system()  — denied function imported directly
                elif isinstance(func, ast.Name) and func.id in denied_func_names:
                    findings.append(f"denied call: '{func.id}()' (imported denied function)")

                # attribute call: os.system(), shutil.rmtree() — resolve aliases first
                elif isinstance(func, ast.Attribute) and isinstance(func.value, ast.Name):
                    real_mod = alias_to_module.get(func.value.id, func.value.id)
                    pair = (real_mod, func.attr)
