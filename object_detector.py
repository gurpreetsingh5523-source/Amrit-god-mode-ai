    ...
    def detect_motion(self, video_path: str, threshold=25) -> list:
        try:
            ...
            while True:
                ...
                if diff.mean()>threshold: motion_frames.append(idx)
                prev_gray=gray; idx+=1
            cap.release(); return motion_frames
        except ImportError: return []

    def find_faces(self, image_path: str) -> list:
        try:
            import cv2; img=cv2.imread(image_path)
            gray=cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
            cascade=cv2.CascadeClassifier(cv2.data.haarcascades+"haarcascade_frontalface_default.xml")
            faces=cascade.detectMultiScale(gray,1.1,4)
            return [{"x":int(x),"y":int(y),"w":int(w),"h":int(h)} for x,y,w,h in faces]
        except ImportError: return []
        except Exception as e: logger.error(f"Face detection error: {e}"); return []

    def describe_with_llm(self, image_path: str, orchestrator=None) -> str:
        if not orchestrator: return "No orchestrator available"
        vision = orchestrator.get_agent("vision")
        if not vision: return "No vision agent"
        import asyncio
