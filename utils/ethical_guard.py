...
class EthicalGuard:
    ...
    def check(self, action: str) -> tuple:
        ...
        # Gurmat/Naam ethics layer — intent-level principle check.
        if self._naam:
            ok, principle, _ = self._naam.check(action)
            if not ok:
                return self._block(action, f"Naam: {principle}")
        return True,"allowed"
