...
class ResearchAgent(BaseAgent):
    ...
    async def _search(self, query: str) -> dict:
        ...
        try:
            ...
            # Blocking network call in a thread — never freeze the async loop/swarm.
            def _fetch():
                with urllib.request.urlopen(req, timeout=8) as r:
                    return r.read().decode("utf-8", errors="ignore")
            html = await asyncio.wait_for(asyncio.to_thread(_fetch), timeout=12)
        ...
        try:
            ...
            def _read():
                with urllib.request.urlopen(req, timeout=10) as r:
                    return r.read().decode("utf-8", errors="ignore")
            html = await asyncio.wait_for(asyncio.to_thread(_read), timeout=15)
