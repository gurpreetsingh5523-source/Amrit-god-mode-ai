...
class ToolAgent(BaseAgent):
    def __init__(self, eb, state):
        ...
        # Safety subsystems (previously orphaned — now wired into execution).
        try:
            from code_safety import CodeSafety
            self.code_safety = CodeSafety()
        except Exception:
            self.code_safety = None
        try:
    ...
    async def execute(self, task: dict) -> dict:
        ...
        tool = d.get("tool")
        if not tool:
            if d.get("action") == "run" or "command" in d:
                tool = "terminal"
            else:
                tool = "file"
    ...
    async def _terminal(self, d):
        ...
        # Layer 3: SafetyLayer risk classification (rm -rf /, sudo, format → BLOCK).
        if self.safety_layer:
            risk = self.safety_layer.classify(cmd)
            if getattr(risk, "value", "") == "block":
                return self.err(f"Action FAILED: Blocked by SafetyLayer — '{cmd[:40]}' "
                                f"classified as destructive. Find a safer alternative.")
            if getattr(risk, "value", "") == "confirm":
                await self.report(f"⚠️ SafetyLayer: '{cmd[:40]}' is CONFIRM-level (proceeding, audited)")

        await self.report(f"$ {cmd}")
        try:
            import asyncio
            # Run blocking subprocess in a thread so the async event loop
            # (and the whole swarm) is NOT frozen during the command.
            def _run():
                return subprocess.run(cmd, shell=True, capture_output=True, text=True,
                                      timeout=d.get("timeout", 60), cwd=d.get("cwd", "workspace"))
    ...
    async def _run_python(self, d):
        ...
        # Layer 2: static code-safety scan. EthicalGuard above is the primary
        # block; CodeSafety adds an AST check and blocks on CRITICAL findings,
        # logging lower-severity warnings without stopping normal code.
        if self.code_safety:
            verdict = self.code_safety.analyze(code)
            if verdict.get("critical_count", 0) > 0:
                return self.err(f"Action FAILED: Blocked by CodeSafety — "
                                f"critical risks: {verdict.get('issues')}")
            if verdict.get("issues"):
                await self.report(f"⚠️ CodeSafety warnings: "
                                  f"{[i.get('call') or i.get('module') for i in verdict['issues']]}")
        # Layer 3: Sandbox forbidden-call check (blocks os.system/eval/exec etc.).
        if self.sandbox:
            ok_sb, why = self.sandbox.is_safe(code)
            if not ok_sb:
                return self.err(f"Action FAILED: Blocked by Sandbox — {why}")
        if self._terminal_ops:
            import asyncio
            result = await asyncio.to_thread(
                self._terminal_ops.run_python, code, d.get("timeout", 15)
            )
