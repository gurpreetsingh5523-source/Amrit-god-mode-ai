...
class InternetAgent(BaseAgent):
    ...
    async def _search(self, query: str) -> dict:
        ...
        # Primary: ddgs package (reliable JSON API)
        try:
            try:
            ...
            self.logger.info(f"DDG search: {len(results)} results for {query!r}")
        except Exception as e:
            self.logger.warning(f"DDG package search failed: {e} — trying HTML fallback")
        ...
        if not results:
            try:
                ...
                html = await self._fetch_text(req, timeout=10)
                ...
                self.logger.warning(f"HTML search fallback failed: {e}")
    ...
    async def _crawl(self, url: str, depth: int = 1) -> dict:
        ...
        async def crawl_page(u, d):
            ...
            try:
                ...
                html = await self._fetch_text(req, timeout=8)
    ...
    async def _wikipedia(self, query: str) -> dict:
        try:
            ...
            # Wikipedia REST API REQUIRES a descriptive User-Agent (else 403).
            req = urllib.request.Request(url, headers={
                "User-Agent": "AmritGodMode/3.0 (https://github.com/gurpreetsingh5523-source/Amrit-god-mode-ai; contact via github)",
                "Accept": "application/json",
            })
            data = await self._fetch_json(req, timeout=8)
    ...
    async def _arxiv(self, query: str, max_results: int = 5) -> dict:
        ...
        try:
            ...
            xml_data = await self._fetch_text(req, timeout=15)
    ...
    async def _pubmed(self, query: str, max_results: int = 5) -> dict:
        ...
        try:
            ...
            data = await self._fetch_json(req, timeout=10)
            ...
            summaries = await self._fetch_json(req, timeout=10)
