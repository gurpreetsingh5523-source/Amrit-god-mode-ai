...
class ImageReader:
    ...
    def resize(self, path: str, w: int, h: int, out: str=None) -> str:
        ...
        img=Image.open(path).resize((w,h)); out=out or path.replace(".",f"_{w}x{h}.")
        img.save(out); return out

    def to_grayscale(self, path: str, out: str=None) -> str:
        from PIL import Image
        img=Image.open(path).convert("L"); out=out or path.replace(".","_gray.")
        img.save(out); return out

    def crop(self, path: str, box: tuple, out: str=None) -> str:
        from PIL import Image
        img=Image.open(path).crop(box); out=out or path.replace(".","_crop.")
        img.save(out); return out

    def thumbnail(self, path: str, size=(128,128)) -> str:
        from PIL import Image
        img=Image.open(path); img.thumbnail(size)
        out=path.replace(".","_thumb."); img.save(out); return out
