...
class Sandbox:
    def __init__(self, timeout=10, max_out=5000):
        self.timeout=timeout; self.max_out=max_out

    def is_safe(self, code: str) -> tuple:
        for f in FORBIDDEN:
            if f in code: return False, f"Forbidden: {f!r}"
        return True, "ok"

    def run(self, code: str, language="python") -> dict:
        safe, reason = self.is_safe(code)
        if not safe: return {"stdout":"","stderr":f"BLOCKED: {reason}","returncode":-2}
    ...
    def _bash(self, code: str) -> dict:
        ...
        except subprocess.TimeoutExpired: return {"stdout":"","stderr":"Bash timeout","returncode":-1}
        except Exception as e: return {"stdout":"","stderr":str(e),"returncode":-1}
        finally:
            try: os.unlink(tmp)
            except: pass
